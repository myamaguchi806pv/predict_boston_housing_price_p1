...
def explore_city_data(city_data):
	...
	# Size of data?
	print "Size of house data %d" %len(housing_prices)
	# Number of features?
    	print "Size of house features %d" %len(housing_features[0])
	# Minimum value?
	print "Minimum: %f" %np.min(housing_prices)
 	# Maximum Value?
	print "Maximum: %f" %np.max(housing_prices)
	# Calculate mean?
	print "Mean: %f" %np.mean(housing_prices)	
	# Calculate median?
	print "Median: %f" %np.median(housing_prices)		
	# Calculate standard deviation?
	print "Deviation: %f" %np.std(housing_prices)
...
def fit_predict_model(city_data):
	...
	reg = GridSearchCV(regressor, cv=5, param_grid=parameters,scoring=my_scorer_func)
	...
	print "reg.scorer_:"
   	print performance_metric([1,0], [10,-10])
